...
printf "\n\n::::: DOCKER TASK :::::\n"
    ...
    echo "  >> Building Image ($DOCKER_USERNAME/$DOCKER_REPOSITORY:$DOCKER_BUILD_ID)"
        docker-compose build web
        docker tag deeper_web:latest $DOCKER_USERNAME/$DOCKER_REPOSITORY:$DOCKER_BUILD_ID
        ...
        if [ "${DJANGO_ONLY_DEPLOY}" == "" ] || [ "${DJANGO_ONLY_DEPLOY}" == "${TYPE}" ] ; then
            ...
            echo "::::::: Creating additional configs :::::"
                ...
                if [ ${TYPE} == 'web' ]; then
                    ...
                    cp $SAMPLE_DIR/.ebextensions/remote_log.config-sample ./.ebextensions/remote_log.config
                ...
                if [ ${TYPE} == 'web' ]; then
                    echo "      >> Creating nginx.conf"
                        ...
                        cp $SAMPLE_DIR/.ebextensions/nginx.conf-sample ./.ebextensions/nginx.conf
                ...
                echo "      >> Creating .mydockercfg "
                    ...
                    cat $SAMPLE_DIR/.mydockercfg-sample \
                ...
                if [ ${TYPE} == 'web' ]; then
                    echo "      >> Creating Dockerrun.aws.json "
                        cat $SAMPLE_DIR/Dockerrun.aws.json-sample \
                    ...
                    echo "      >> Creating Dockerrun.aws.json "
                        cat $SAMPLE_DIR/DockerrunMulti.aws.json-sample \
                            | sed 's\DEPLOYMENT_BUCKET\'$DEPLOYMENT_BUCKET'\' \
                            | sed 's\DOCKER_AUTH_FILE\'.mydockercfg'\' \
